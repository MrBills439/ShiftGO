...
/**
 * Cover / swap approval queue. Rendered inside the Shifts page (Requests tab)
 * for HR / managers; the parent handles route-level role gating.
 */
export function ShiftRequestsPanel() {
  const [tab, setTab] = useState<(typeof TABS)[number]['key']>('PENDING_MANAGER');
  const { data, isLoading } = usePendingShiftChanges(tab);
