...
export default function TodayPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const activeShifts = todayShifts.filter((s) => new Date(s.startTime) <= now && new Date(s.endTime) >= now);
    const lateWorkers = todayShifts.filter((s) => new Date(s.startTime) <= now && new Date(s.endTime) >= now && s.status === 'SCHEDULED');
    const openShifts = todayShifts.filter((s) => s.status === 'SCHEDULED' && new Date(s.startTime) > now);
    const coverage = todayShifts.length > 0 ? Math.round((activeShifts.length / todayShifts.length) * 100) : 100;
  ...
  const agencyName = 'Ramsay Care Group'; // TODO: Get from user/agency context
    ...
    <div className="space-y-8">
            ...
            <>
              {metrics.lateWorkers.map((shift) => (
                <ProblemAlert
                  key={`late-${shift.id}`}
                  severity="warning"
                  title={`${shift.worker.name} is late`}
                ...
                <div key={shift.id} className="p-4 flex items-center justify-between hover:bg-neutral-50 transition-colors">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <p className="font-semibold text-fg">{shift.worker.name}</p>
                      <Badge
                        variant={
                  ...
                  <div className="pt-4 border-t border-neutral-200 space-y-2">
                    {tomorrowShifts.slice(0, 3).map((shift) => (
                      <div key={shift.id} className="flex items-center justify-between text-sm">
                        <span className="text-fg">{shift.worker.name}</span>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <Button variant="primary" className="w-full">Create Shift</Button>
          <Button variant="primary" className="w-full">Emergency Shift</Button>
          <Button variant="secondary" className="w-full">Assign Worker</Button>
          <Button variant="secondary" className="w-full">Create Worker</Button>
          <Button variant="secondary" className="w-full">Create House</Button>
        </div>
      </section>
